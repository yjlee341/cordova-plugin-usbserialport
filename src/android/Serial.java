...
public class Serial extends CordovaPlugin {
	...
	private static final String ACTION_IS_OPEN = "isOpen";
	...
	public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
		...
		// get All devices
		else if(ACTION_GET_DEVICE_INFO.equals(action)){
		...
		// open status
		else if(ACTION_IS_OPEN.equals(action)) {
		...
		cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				try{
					...
					deviceInfo.put("devClass", device.getDeviceClass());
					deviceInfo.put("devSubClass", device.getDeviceSubclass());
					deviceInfo.put("devProtocol", device.getDeviceProtocol());
					deviceInfo.put("interface", device.getInterfaceCount());
	...
	private void isOpen(final CallbackContext callbackContext) {
		cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				...
				try{
					openStatus = port.isOpen() == true ? true : false;
					PluginResult result = new PluginResult(PluginResult.Status.OK, openStatus);
			...
			public void run() {
				if (port == null) {
					...
					try {
						...
						port.write(buffer, 1000);
						callbackContext.success(data + " bytes written.");
					}
					catch (IOException e) {
						// deal with error
						Log.d(TAG, e.getMessage());
